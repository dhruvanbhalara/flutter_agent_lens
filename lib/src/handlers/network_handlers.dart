...
extension NetworkHandlers on FlutterAgentLensServer {
  ...
  Future<CallToolResult> _handleStartNetworkCapture(CallToolRequest req) async {
    ...
    try {
      ...
      return CallToolResult(
        content: [TextContent(text: 'Failed to start network capture: $e')],
        isError: true,
      );
    }
  }
  ...
  Future<CallToolResult> _handleStopNetworkCapture(CallToolRequest req) async {
    ...
    try {
      ...
      final slowRequests = completedRequests
          .where(
              (r) => ((r['endTime'] as int) - (r['startTime'] as int)) > 2000)
          .toList();
      ...
      stderr.writeln('[mcp:network_capture] ERROR: $e');
      stderr.writeln('[mcp:network_capture] STACKTRACE: $st');
      return CallToolResult(
        content: [TextContent(text: 'Failed to stop network capture: $e')],
        isError: true,
      );
    }
  }
