...
void main() {
  ...
  group('isDtdUri', () {
    test('identifies direct VM Service URIs', () {
      ...
      expect(
          support.isDtdUri('http://127.0.0.1:8181/auth_token=/ws/'), isFalse);
      expect(
          support.isDtdUri('http://127.0.0.1:8181/ws?auth_token=abc'), isFalse);
  ...
  group('normalizeToWsUri', () {
    test('converts http/https to ws/wss', () {
      expect(support.normalizeToWsUri('http://127.0.0.1:8181/auth_token=/ws'),
          equals('ws://127.0.0.1:8181/auth_token=/ws'));
      expect(support.normalizeToWsUri('https://127.0.0.1:8181/auth_token=/ws'),
          equals('wss://127.0.0.1:8181/auth_token=/ws'));
    });

    test('leaves ws/wss unchanged', () {
      expect(support.normalizeToWsUri('ws://127.0.0.1:8181/auth_token=/ws'),
          equals('ws://127.0.0.1:8181/auth_token=/ws'));
      expect(support.normalizeToWsUri('wss://127.0.0.1:8181/auth_token=/ws'),
          equals('wss://127.0.0.1:8181/auth_token=/ws'));
    });

    test('adds /ws suffix to direct VM Service URIs if missing', () {
      expect(support.normalizeToWsUri('http://127.0.0.1:8181/auth_token='),
          equals('ws://127.0.0.1:8181/auth_token=/ws'));
    });

    test('does not add /ws suffix to DTD URIs', () {
      expect(support.normalizeToWsUri('http://127.0.0.1:6262'),
